...
const subscribeWithWallet = async (req, res) => {
  try {
    ...
    if (req.user.WalletBalance >= package.SubscriptionPrice)
      var updatePatient = await patientModel.findByIdAndUpdate(req.user.id, {
        WalletBalance: req.user.WalletBalance - package.SubscriptionPrice,
        Wallet: req.user.Wallet - package.SubscriptionPrice,
      });
    else return res.status(500).json("Insufficient");
...
const addSubscription = async (req, res) => {
  try {
    const { packageId } = req.body;
    const subscribedAlready = await subscriptionModel.findOne({
      Patient: req.user.id,
    });
    ...
    return res
      .status(201)
      .json({ message: "Subscription added successfully", newSubscription });
  } catch (error) {
    return res.status(400).json({ error: error.message });
  }
};
...
const deleteOneSubscription = async (req, res) => {
  try {
    ...
    if (subscriptions.length == 2)
      var deletedSubscription = await subscriptionModel.deleteOne({
        Patient: req.user.id,
      });
  } catch (error) {
    return res.status(400).json({ error: error.message });
