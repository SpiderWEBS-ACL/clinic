...
import { Button, DatePicker, DatePickerProps, Input, Modal, Row, Select, TimePickerProps, message } from 'antd';
...
const ViewPatientInfo = () => {
  ...
  const handleOk = async () => {
    ...
    const response = await api.post(`/doctor/addHealthRecordForPatient/${id}`, data ,config);
  }
  else{
  ...
  const [year, month, day] =String(patientInfo.Dob).substring(0, 10).split('-');
  ...
  const followup = async()=>{
    ...
    const response = await api.post(`/doctor/scheduleFollowup/`, {
       Patient: id,
       appDate: date,
       followUp: true,
       status: "Upcoming"
  ...
  >
    ...
    <TextArea rows={4} placeholder="Enter description..." maxLength={200} onChange={handleInputChange}/>
...
>  {healthRecords.map((record, index) => (
            <div>
              ...
              >
                <Meta
                  avatar={<Avatar sx={{bgcolor: green[500] }}>   <AssignmentIcon />  </Avatar>  }
